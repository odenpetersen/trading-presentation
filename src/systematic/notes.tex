...
\RenewDocumentCommand{\section}{s o m}{%
    ...
    {\centering\textit{\insertsquote}\par}%
...
No AI writing has been used here. This is the result of reading and experience.

I originally presented this as a talk to around 20 students at UNSW.

Links: \textcolor{blue}{\href{https://github.com/odenpetersen/trading-presentation}{GitHub Repo, Other talks}}, \textcolor{blue}{\href{https://www.youtube.com/watch?v=hYmR5qk4z8w}{Talk Recording}} (1h59m), \textcolor{blue}{\href{https://linkedin.com/in/oden-petersen}{About me}}
...
\section*{Not the Point of This Talk}
\begin{itemize}
...
\subsection{Securities Markets and Exchanges}
...
\subsubsection{Collateralisation}
At certain intermediate times $t'$ ($t\leq t'\leq T$), participants may be required to physically give (`post') something to the exchange to \textbf{collateralise} their obligations. This could be money (`margin') or other assets (`locate'/`borrow').
...
\subsubsection{Portfolio Valuation}
...
where $p_\omega$ is now a vector-valued measure. Let
...
\subsubsection{Properties of Returns and Log-Returns}
...
\subsection{Limit Order Book}%Special kind of data structure
...
\subsubsection{Supply and Demand Curves}
...
\subsubsection{Batch Matching Properties}
...
\subsubsection{Continuous Matching}
...
\subsection{Trades Resulting from Continuous Matching}
...
\subsubsection{Instantaneous Price Impact}
...
It is well known that the last traded price $\$p_t$ exhibits \textbf{negative serial autocorrelation} of returns $\frac{dp}{p_t^-}$ (sometimes called \textbf{bid-ask bounce}).
...
\subsubsection{Common Microprices}
...
\subsubsection{Persistent Price Impact}
...
\subsubsection{P\&L with transaction costs}
...
\subsection{Downsides of Market Making}
...
\subsection{Market Making Strategy}
...
\subsection{Markowitz Portfolio Optimisation}
...
\subsection{Spanned and Orthogonal $\alpha$}
...
\subsubsection{Pervasiveness of $\alpha^\top$}
...
\subsubsection{``Rotational'' Indeterminacy}
...
\subsubsection{Fundamental Factor Models}
...
It is usually best to ensure the characteristics are close to linearly independent.
...
Let $F=[r_1^A x_1, r_2^A x_2,\ldots, r_n^A x_n]^\top$ and $\overline{F} = \frac{1}{n} \sum_{i=1}^n r_i^A x_i$.
...
\subsection{Sharpe Ratio Hypothesis Testing }
...
\subsection{Transaction Costs}
\subsubsection{Transaction Cost Analysis}
Recall that
$$\$\Pi_T	= \underbrace{\$\int_{t\in T} a_t^- dm^\emptyset}_{\textrm{Midprice P\&L}} - \underbrace{\$\int_{t\in T} \nu_t da}_{\textrm{PPI Penalty}} - \underbrace{\$\int_{t\in T} \lambda_t da}_{\textrm{IPI Penalty}}.$$
...
\subsubsection{Transaction Cost Optimisation}
...
\subsection{Asymptotic Filtering}
...
\subsection{Proof Sketch for $c_T$ Identity}
\begin{align*}
...
\subsection{Alternative Price Proxies}
...
\subsection{Stabilising $\hat{B}_{\textrm{PPCA}}$}
...
\subsection{Observation and Transition}
...
\subsection{Dutch Book Theorems}
...
\subsection{$\mathbb{Q}$-world vs $\mathbb{P}$-world}
...
\subsection{Useful}
\begin{itemize}
	\item Richard C. Grinold and Ronald N. Kahn. \textit{Active Portfolio Management}. Probus, 1994.
	\item James D. Hamilton. \textit{Time Series Analysis}. Princeton University Press, 1994.
	\item A. P. Dempster, N. M. Laird, and D. B. Rubin. ``Maximum Likelihood from Incomplete Data via the EM Algorithm.'' \textit{Journal of the Royal Statistical Society: Series B} 39(1), 1977.
	\item Michael E. Tipping and Christopher M. Bishop. ``Probabilistic Principal Component Analysis.'' \textit{Journal of the Royal Statistical Society: Series B} 61(3), 1999.
	\item Joel Hasbrouck. \textit{Empirical Market Microstructure}. Oxford University Press, 2006.
	\item Marco Avellaneda and Sasha Stoikov. ``High-frequency Trading in a Limit Order Book.'' \textit{Quantitative Finance} 8(3), 2008.
	\item Trevor Hastie, Robert Tibshirani, and Jerome Friedman. \textit{The Elements of Statistical Learning}. 2nd ed. Springer, 2009.
	\item Barry Johnson. \textit{Algorithmic Trading and DMA}. 4Myeloma Press, 2010.
	\item Jim Gatheral. ``Three Models of Market Impact.'' 2016.
	\item Jean-Philippe Bouchaud, Julius Bonart, Jonathan Donier, and Martin Gould. \textit{Trades, Quotes and Prices}. Cambridge University Press, 2018.
	\item Richard C. Grinold and Ronald N. Kahn. \textit{Advances in Active Portfolio Management}. McGraw-Hill, 2019.
	\item Patrice Bertail and Emmanuelle Gautherat. ``Exponential Bounds for Regularised Hotelling's $T^2$ Statistic in High Dimension.'' 2024.
	\item Giuseppe A. Paleologo. \textit{The Elements of Quantitative Investing}. Wiley, 2025.
\end{itemize}

\subsection{Classics \& Mentioned}
\begin{itemize}
	\item Frank P. Ramsey. ``Truth and Probability.'' 1926. In \textit{The Foundations of Mathematics and Other Logical Essays}, Routledge \& Kegan Paul, 1931.
	\item John von Neumann and Oskar Morgenstern. \textit{Theory of Games and Economic Behavior}. 2nd ed. Princeton University Press, 1947.
	\item A. N. Kolmogorov. \textit{Foundations of the Theory of Probability}. Chelsea, 1950.
	\item Harry Markowitz. ``Portfolio Selection.'' \textit{The Journal of Finance} 7(1), 1952.
	\item Kenneth J. Arrow and Gerard Debreu. ``Existence of an Equilibrium for a Competitive Economy.'' \textit{Econometrica} 22(3), 1954.
	\item J. L. Kelly, Jr. ``A New Interpretation of Information Rate.'' \textit{Bell System Technical Journal} 35(4), 1956.
	\item R. E. Kalman. ``A New Approach to Linear Filtering and Prediction Problems.'' \textit{Journal of Basic Engineering} 82(1), 1960.
	\item Jack L. Treynor. ``Market Value, Time, and Risk.'' Unpublished manuscript, 1961.
	\item A. N. Tikhonov. ``Solution of Incorrectly Formulated Problems and the Regularization Method.'' \textit{Soviet Mathematics Doklady} 4, 1963.
	\item William F. Sharpe. ``Mutual Fund Performance.'' \textit{The Journal of Business} 39(1), 1966.
	\item Eugene F. Fama and James D. MacBeth. ``Risk, Return, and Equilibrium: Empirical Tests.'' \textit{Journal of Political Economy} 81(3), 1973.
	\item J. Michael Harrison and David M. Kreps. ``Martingales and Arbitrage in Multiperiod Securities Markets.'' \textit{Journal of Economic Theory} 20(3), 1979.
	\item Sanford J. Grossman and Joseph E. Stiglitz. ``On the Impossibility of Informationally Efficient Markets.'' \textit{The American Economic Review} 70(3), 1980.
	\item Jerome H. Friedman. ``Multivariate Adaptive Regression Splines.'' \textit{The Annals of Statistics} 19(1), 1991.
	\item Yakov Amihud. ``Illiquidity and Stock Returns: Cross-Section and Time-Series Effects.'' \textit{Journal of Financial Markets} 5(1), 2002.
\end{itemize}
\end{document}
