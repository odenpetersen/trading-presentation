...
with open("container_latency.csv") as f:
	for row in csv.DictReader(f):
		rows[(row["container"], int(row["n"]))].append(int(row["cycles"]))
...
for name, (label, color) in series.items():
	ns = sorted({n for (c, n) in rows if c == name})
	med = np.array([np.median(rows[(name, n)]) for n in ns])
	p25 = np.array([np.percentile(rows[(name, n)], 25) for n in ns])
	p75 = np.array([np.percentile(rows[(name, n)], 75) for n in ns])
	ax.fill_between(ns, p25, p75, color=color, alpha=0.15, linewidth=0)
	ax.plot(ns, med, color=color, linewidth=2)
	ax.annotate(label, xy=(ns[-1], med[-1]), xytext=(6, 0), textcoords="offset points",
		color=color, fontsize=10, va="center", fontweight="bold")

ax.set_xscale("log")
ax.set_yscale("log")
ax.set_xlabel("container size n")
ax.set_ylabel("cycles per contains() call, cold cache")
