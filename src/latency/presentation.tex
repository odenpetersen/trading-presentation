...
\begin{frame}{Network Timestamping}
	...
	Usually the timestamp is calculated according to the first bit of the Ethernet header (start-of-frame). The last bit can be calculated given the bandwidth of the transmission.
...
\begin{frame}{Publication Latency}
	...
	Larger messages may be \textbf{packetised} and packets may be \textbf{fragmented} (according to the L2/L3 Maximum Transmission Unit).
	%Can correlate with things we don't see
\end{frame}

\begin{frame}{Distribution Latency}
	\begin{block}{Public Market Data}
	...
	\begin{block}{Private Feed}
		This is typically \textbf{unicast}, and the matching engine may need to decide on some reasonable order in which to send updates to participants. Depending on how this is achieved, there might be ways to gain priority in the distribution of private feed messages.
	\end{block}
\end{frame}
...
\begin{frame}{Network Interface Cards}
	...
	%A standard `dumb' NIC will have two \textbf{network packet buffers}, one for \textbf{RX (receive)} and one for \textbf{TX (transmit)}.

	%%picture of PCIe cable
	%\begin{block}{Programmed Input/Output (PIO)}
		%Incoming packets will first be stored in a \textbf{network packet buffer} on the NIC, then handed off to the CPU via an interconnect such as \textbf{PCIe}. This involves an \textbf{interrupt request} to be handled by the OS kernel.

		%The PCIe standard defines a physical databus architecture and a packetisation format, including a special `\textbf{interrupt request}' message that asks the OS kernel to poll the data from the NIC buffer. Data will then be serially transmitted along a PCIe connection to the CPU, and serially written to RAM if register capacity is exceeded.
	%\end{block}

	%\begin{block}{Direct Memory Access (DMA)}
		%Modern setups will generally allow for Direct Memory Access (DMA), whereby the \textbf{RX (receive) queue} is streamed directly into a dedicated section of RAM, or the \textbf{TX (transmit) queue} is populated directly from RAM.
	%\end{block}
	%%memory controller
	%%ddio
\end{frame}
