...
\begin{frame}{Electronic Trading}
	\begin{itemize}
		...
		\begin{itemize}
			\item \textbf{Insert} an order with some \textbf{lifetime}
...
\begin{frame}{Electronic Trading}
	%do some EDA on book dynamics.
	%market moved vs theo moved, hitting on insert vs hitting on other book event
%fill rate
...
\begin{frame}{Clock Drift}
	...
	%graph for ES, control for things
...
\begin{frame}{Externalities}
	...
	\begin{itemize}
		\item \textbf{Load balancers} help accomodate the large volume of data moving through the network, particularly during significant market events.
	\end{itemize}
...
\begin{frame}{Controllable Logic Blocks (CLBs)}
	\begin{center}
...
\begin{frame}{Memory}
	...
	\begin{center}
		\includegraphics[scale=0.1]{assets/dflipflop.png}
	\end{center}
\end{frame}
...
\end{lstlisting} %might have multiple read/write ports
	...
	B-RAM is a kind of \textbf{Static Random Acccess Memory} (S-RAM). Dedicated RAM devices are typically cheaper, slower \textbf{Dynamic Random Access Memory} (D-RAM).

	Some RAMs support \textbf{double-data input/output} (DDIO), moving data at both positive and negative clock edges.
...
\begin{frame}{Synthesis}
	...
	Sophisticated software also exists to translate higher-level descriptions (for instance, code written in a dialect of C) into RTL designs. This is known as \textbf{High-Level Synthesis} (HLS).
...
\begin{frame}{Microarchitecture}
	\centering
	\includegraphics[scale=0.7]{assets/intel.png}
\end{frame}
...
\begin{frame}{Out-of-Order Execution}
	...
	CPUs are not always transparent about their inner workings, but tools like \texttt{llvm-mca --timeline} can give some very rough hints.
%mca demo in godbolt
...
\begin{frame}{x86}
	An \textbf{Instruction Set Architecture} defines the available machine instructions and their behaviour.
...
\begin{frame}{Heap Memory}
	\begin{itemize}
		...
		\item We can request \textbf{virtual memory} from the OS using syscalls like \texttt{brk} and {mmap}
...
\begin{frame}{Cache Misses}
	...
	\includegraphics[scale=0.2]{code/cache/cdf.png}
...
\begin{frame}{Branch Misses}
	\begin{itemize}
		\item We can count branch missses with \texttt{perf stat -e branches,branch-misses}
...
\begin{frame}{Profiling}
	\begin{itemize}
		...
		\item \texttt{\_\_rdtsc()} + \texttt{\_mm\_mfence()}
	\end{itemize}
\end{frame}
...
\begin{frame}{Multi-Core Processing}
	\begin{itemize}
		...
		\item \textbf{Interrupt Request} (IRQ) affinity (particularly NIC IRQs)
		\item NIC IRQ Receive Packet Steering (RPS) can help with load balancing
	...
	\begin{block}{Interprocess Communication (IPC)}
		Threads often need to communicate information to one another.
...
\begin{frame}{Cache Coherence}
	\begin{itemize}
		...
		\item Cache writes take some time to commit to memory, so other threads may not see the write immediately
...
\begin{frame}{Kernel Networking}
	\includegraphics[scale=0.5]{assets/kernel_tx.png}
...
\begin{frame}{Userspace Networking}
	\begin{itemize}
		...
		\item \textbf{Continuous polling} of the RX queue, combined with \textbf{cache warming}, allows for rapid response
	\end{itemize}

	\begin{block}{Kernel Bypass Frameworks}
		We will need to know the layout of DMA to communicate with the NIC.

		Frameworks like TCPDirect, DPDK (+F-Stack/mTCP) provide specialised APIs for kernel bypass.

		OpenOnload, VMA (vhost/Mellanox Messaging Accelerator) make use of \texttt{LD\_PRELOAD} to monkeypatch\footnote{Specifically, changing the dynamic linking} the kernel's networking API.
	\end{block}
\end{frame}
...
\begin{frame}{Compilation}
	\begin{block}{Development}
		...
		\begin{itemize}
			\item Optimisation levels, \texttt{-O0} (default) to \texttt{-O7}
