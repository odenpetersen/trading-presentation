...
\begin{frame}{Fragmentation}
	...
	\end{block}
	%Fragments will only be held for so long before being dropped
	...
	%favours small frames/packets, but then need more ethernet frame metadata including preamble. work through the maths
\end{frame}
...
\begin{frame}{Public Market Data}
	...
	\begin{block}{Public Market Data}
		``At one later site in the new KRX Annexe, we put in market data and it was the worst. \ldots We asked for this market data to be removed. A month or two later we paid for it to be reinstalled \ldots and now it was one of the best. Such were the vagaries of life in Korea. I suspect some new kind of semi-permanent \textbf{round robin} positioning somewhere fixed things. There was little rhyme nor reason for many things.''
	\end{block}
...
%timing. restricted by line bandwidth in some ways
\end{frame}

\begin{frame}{Synchronous Design}
%clocks, clock edges, overclocking. 
\end{frame}

\begin{frame}{Parallelism and Pipelining}
%misd. "pipelining is bad" = dont do stuff in serial
%how much speedup from parallelisation
%Packet slicing, only read part of packet
\end{frame}

\begin{frame}{Memory}
%memory: flipflops, bram, ultraram, registers, external dram, memory bandwidth, read/write ports count, pipeline depth
\end{frame}
...
%cost model
%optimisation methods like simulated annealing or whatever's actually more common
%area becomes a problem when you run out of LUTs or need to slow down timing
	%One concrete example of an FPGA is the \href{https://docs.amd.com/r/en-US/Alveo-X3522PV-Adaptable-Accelerator-Card-User-Guide-UG1607/}{X3522PV Accelerator Card}.
	%can see how much of each resource is available
\end{frame}

%Hardware is expensive to buy, expensive to program, expensive to maintain. 

\begin{frame}{Interconnects}
%interconnects
	%picture of PCIe cable

	%This involves an \textbf{interrupt request} to be handled by the OS kernel.
